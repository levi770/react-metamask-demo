...
const App = () => {
  ...
  const handleLogin = async (address) => {
    ...
    if (!nonce) { throw new Error("Invalid message to sign") } 
  ...
  const requestTx = async (tx) => {
    if (tx.amount.trim() === "" ||
      ...
      return {
        success: false,
        status: <p id="status" style={{ color: "red" }}>Please make sure all fields are completed before sending request.</p>
      };
    }

    try {
      const txResponse = await axios.post(`${baseUrl}/wallet/transfer`, tx, { headers: { "Authorization": `Bearer ${token}` } });
      ...
      return {
        success: true,
        status: <p id="status" style={{ color: "green" }}>{ str }<a href={url} target="_blank" rel="noreferrer">{url}</a></p>
      }
    } catch (error) {
      const str = "😥 Something went wrong: " + error.message
      return {
        success: false,
        status: <p id="status" style={{ color: "red" }}>{ str }</p>
      };
    }
  };
  ...
  return (
      <div className="App">
        ...
        <p>
          This is a demo of a ReactJS app that uses the Metamask browser extension and REST API to authenticate users
          by signing a message with nonce received from server and their private key,
          than server recovers user's address from provided signature and return JWT token to React App.
          After JWT token is received, React App can interact with servers private endpoints and generate transactions.
        </p>
